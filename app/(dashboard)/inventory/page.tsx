...
export default function InventoryScreen() {
  ...
  return (
    <div className="max-w-7xl mx-auto w-full">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {inventory.map(item => (
            <motion.div variants={itemVariants} key={item.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex justify-between items-center hover:shadow-md transition">
              <div>
                <p className="text-xs font-black uppercase tracking-widest text-neutral-400 mb-1">Product</p>
                <h3 className="text-xl font-bold text-pesa-navy">{item.name.replace('Product Name ', '')}</h3>
      ...
      <AnimatePresence>
        {isAddOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-pesa-navy/60 backdrop-blur-sm z-50 flex items-center justify-center px-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
